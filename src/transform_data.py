...
path_name = Path(__file__).parent.parent / 'data' / 'weather_data.json' # é a mesma coisa que escrever ../data/weather_data.json
...
def create_dataframe(path_name:str) -> pd.DataFrame:
    logging.info("Criando DataFrame JSON...")
    path = Path(path_name)
...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
    df_weather = pd.json_normalize(df['weather'].apply(lambda x: x[0])) # passa por cada linha da coluna e pega o primeiro item
